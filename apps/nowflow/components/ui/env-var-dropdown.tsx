...
export const EnvVarDropdown = ({
  ...
  return (
    <div
      className={cn(
        'workflow-editor-menu-content workflow-editor-portal-surface absolute z-[9999] w-full mt-1 overflow-hidden bg-popover rounded-md border shadow-md',
        className
      )}
      style={style}
    >
      {filteredEnvVars.length === 0 ? (
        <div className="workflow-editor-menu-empty px-3 py-2 text-sm text-muted-foreground">
        ...
        <div className="py-1">
          {filteredEnvVars.map((envVar, index) => (
            <button
              key={envVar}
              className={cn(
                'workflow-editor-menu-item w-full px-3 py-1.5 text-sm text-left',
