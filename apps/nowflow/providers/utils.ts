...
> = {
  ...
  google: {
    ...
    models: [
      ...
      'gemini-2.0-flash', // Retiring June 2026
      'gemini-2.0-flash-lite', // Retiring June 2026
    ],
  ...
  groq: {
    ...
    modelPatterns: [/^llama/, /^qwen/, /^deepseek-r1-distill/],
  },
  ollama: {
    ...ollamaProvider,
    models: [],
...
Object.entries(providers).forEach(([id, provider]) => {
  if (provider.initialize && id !== 'ollama') {
...
// Ollama models are persisted in localStorage via useOllamaStore (with zustand persist).
// On page load, the store rehydrates from localStorage and syncs to providers.ollama.models
// via the onRehydrate callback. No server-side auto-fetch needed at startup.

// Function to update Ollama provider models
export function updateOllamaProviderModels(models: string[]): void {
  providers.ollama.models = models
  logger.debug('Updated Ollama provider models', { models })
}
...
export function getApiKey(provider: string, model: string, userProvidedKey?: string): string {
  ...
  if (provider === 'ollama') {
    return 'ollama' // Return a dummy key for Ollama
  }
