...
export interface ExecutionMetadata {
  ...
  workflowConnections?: WorkflowEdgeConnection[] // Connections between workflow blocks
  // HITL pause information
  hitlPaused?: boolean // Whether the workflow is paused for human approval
  hitlRequestId?: string // ID of the HITL request
  hitlBlockId?: string // ID of the block that triggered the pause
  // A/B Experiment tracking
  experimentId?: string // ID of the active experiment
  variantId?: string // ID of the selected variant
  variantName?: string // Name of the selected variant for display
}

export interface WorkflowEdgeConnection {
  source: string
  target: string
...
export interface ExecutionContext {
  ...
  executionId?: string // Unique identifier for this specific execution (for memory tracking)

  // User/Session tracking for agent memory (NEW - backward compatible)
  userId?: string // Authenticated user ID (optional)
  sessionId?: string // Session identifier for memory isolation (optional)
  sessionToken?: string // Session token from X-Session-Token header (optional)

  // Memory configuration for deployment mode (NEW - for chat deployments)
  memoryEnabled?: boolean // Override memory enabled for deployment
  sessionMetadata?: Record<string, any> // Deployment session metadata (IP, fingerprint, etc.)

  // API configuration for deployment subdomains (NEW - for fetch to work on subdomains)
  apiBaseUrl?: string // Base URL for API calls (e.g., https://subdomain.example.com)

  // Workflow ownership and notification preferences (NEW - for email notifications)
  ownerId?: string // User ID of the workflow owner (for notifications)
  notifyOnCompletion?: boolean // Whether to send completion notification
  notifyOnFailure?: boolean // Whether to send failure notification
  ...
  edges?: WorkflowEdgeConnection[] // Workflow edge connections
}
...
export interface ExecutionResult {
  ...
  // Block identification (used in streaming responses)
  blockId?: string
  blockName?: string
  blockType?: string
}
