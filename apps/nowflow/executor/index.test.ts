...
import { normalizeBlockOutput } from './normalization'
...
vi.mock('@/stores/execution/store', () => ({
  useExecutionStore: {
    getState: () => ({
      ...
      setExecutionTiming: vi.fn(),
      setExecutionStatus: vi.fn(),
      setCompletedBlocks: vi.fn(),
      setErrorBlocks: vi.fn(),
      setBlockMetrics: vi.fn(),
...
vi.mock('./handlers', () => {
  // Factory function for handler mocks (constructable via `new`)
  ...
  // HITLPauseError needs to be a real class since executor uses `instanceof` checks
  class HITLPauseError extends Error {
    requestId: string
    blockId: string
  ...
  return {
    ...
    HumanAgentBlockHandler: createHandler('human-agent'),
    ...
    ApprovalBlockHandler: createHandler('approval'),
    SubWorkflowBlockHandler: createHandler('sub-workflow'),
    GenericBlockHandler: createHandler('generic'),
    HITLPauseError,
    buildPersonaSystemPrompt: vi.fn(() => ''),
  }
})

// Mock the PathTracker
vi.mock('./path', () => ({
  PathTracker: vi.fn(function () {
    return {
...
const createSerializedBlock = (metadataId: string): SerializedBlock => ({
  id: `${metadataId}-block`,
  position: { x: 0, y: 0 },
  config: { tool: 'test-tool', params: {} },
  inputs: {},
...
describe('Executor', () => {
  ...
  describe('normalizeBlockOutput', () => {
    test('should normalize different block outputs correctly', () => {
      ...
      // Use the extracted normalizeBlockOutput function directly
      const normalizeOutput = normalizeBlockOutput

      // Test normalizing agent block output
      const agentBlock = createSerializedBlock('agent')
      ...
      const routerBlock = createSerializedBlock('router')
      ...
      const functionBlock = createSerializedBlock('function')
      ...
      const genericBlock = createSerializedBlock('unknown')
    ...
    test('should normalize error outputs correctly', () => {
      ...
      const normalizedErrorOutput = normalizeOutput(errorOutput, createSerializedBlock('api'))
      ...
      const normalizedResponseError = normalizeOutput(
        responseErrorOutput,
        createSerializedBlock('api')
      )
  ...
  describe('error handling', () => {
    ...
    test('should create proper error output for a block error', () => {
      ...
      // Use the extracted extractErrorMessage function directly
  ...
  describe('loop management', () => {
    ...
    test('should update loop indices correctly between iterations', async () => {
      ...
      vi.doMock('./handlers', () => {
        ...
        return {
          AgentBlockHandler: makeHandler({ canHandle: () => false, execute: vi.fn() }),
          HumanAgentBlockHandler: makeHandler({ canHandle: () => false, execute: vi.fn() }),
          RouterBlockHandler: makeHandler({ canHandle: () => false, execute: vi.fn() }),
          ConditionBlockHandler: makeHandler({ canHandle: () => false, execute: vi.fn() }),
          EvaluatorBlockHandler: makeHandler({ canHandle: () => false, execute: vi.fn() }),
          FunctionBlockHandler: makeHandler({
            canHandle: (block: any) =>
          ...
          ApiBlockHandler: makeHandler({ canHandle: () => false, execute: vi.fn() }),
          ApprovalBlockHandler: makeHandler({ canHandle: () => false, execute: vi.fn() }),
          SubWorkflowBlockHandler: makeHandler({ canHandle: () => false, execute: vi.fn() }),
          GenericBlockHandler: makeHandler({
            canHandle: () => true,
    ...
    test('should fix the bug where first two iterations showed same index', async () => {
      ...
      const workflow = createWorkflowWithLoop()
      const loopManager = {
        processLoopIterations: async (context: {
        ...
        getLoopIndex: (
          loopId: string,
          _blockId: string,
          context: { loopIterations: Map<string, number> }
        ) => context.loopIterations.get(loopId) || 0,
      }
