  ...
  private async executeInternal(
    ...
    for (const condition of conditions) {
      ...
      try {
        ...
        const conditionMet = await withSpan(
          'executor.condition.evaluate',
          async () =>
            new Function('context', `with(context) { return ${resolvedConditionValue} }`)(
              evalContext
            ),
          {
            blockId: block.id,
        ...
        // Find connection for this condition - support both new and old connection formats
        const connection = outgoingConnections?.find(
          (conn) =>
            conn.sourceHandle === `condition-${condition.id}` ||
            (conn.sourceHandle === 'if' && condition.title === 'if') ||
            (conn.sourceHandle === 'else' && condition.title === 'else')
        ) as { target: string; sourceHandle?: string } | undefined
    ...
    if (!selectedConnection || !selectedCondition) {
      ...
      if (elseCondition) {
        ...
        const elseConnection = outgoingConnections?.find(
          (conn) =>
            conn.sourceHandle === `condition-${elseCondition.id}` || conn.sourceHandle === 'else'
        ) as { target: string; sourceHandle?: string } | undefined
        if (elseConnection) {
