  ...
  private async executeInternal(
    ...
    try {
      const result = await withSpan(
        `executor.tool.${block.config.tool}`,
        async () =>
          executeTool(block.config.tool, {
            ...inputs,
            _context: {
        ...
        { toolId: block.config.tool, blockId: block.id, workflowId: context.workflowId }
      )
