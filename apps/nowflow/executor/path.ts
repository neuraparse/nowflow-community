    ...
    for (const blockId of executedBlockIds) {
      ...
      if (block?.metadata?.id === 'router') {
        ...
        if (selectedPath) {
          ...
          // Also activate utility blocks feeding into the selected path
          this.workflow.connections
            .filter((c) => c.target === selectedPath && c.targetHandle === 'utility-target')
        ...
        if (selectedConditionId) {
          ...
          if (targetConnection) {
            ...
            // Also activate utility blocks feeding into the selected condition target
            this.workflow.connections
              .filter(
                (c) => c.target === targetConnection.target && c.targetHandle === 'utility-target'
              )
              .forEach((c) => context.activeExecutionPath.add(c.source))
        ...
        for (const conn of outgoingConnections) {
          ...
          // Helper to also activate utility blocks when a host block enters the active path.
          // Utility edges: utility_block (utility-source) → host_block (utility-target).
          const activateWithUtilityDeps = (targetId: string) => {
            context.activeExecutionPath.add(targetId)
            this.workflow.connections
              .filter((c) => c.target === targetId && c.targetHandle === 'utility-target')
              .forEach((c) => context.activeExecutionPath.add(c.source))
          }
          ...
          if (conn.sourceHandle === 'error') {
            if (hasError) {
              activateWithUtilityDeps(conn.target)
            }
          }
