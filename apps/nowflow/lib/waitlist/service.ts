...
export async function addToWaitlist(email: string): Promise<{ success: boolean; message: string }> {
  try {
    ...
    try {
      const rendered = await renderEmail('waitlist-confirmation', { email: normalizedEmail })
...
export async function getWaitlistEntries(
  ...
  try {
    ...
    logger.debug(
      ...
      shouldFilterByStatus ? status : 'No status filter'
    )

    // Now build the conditions
    if (shouldFilterByStatus && search && search.trim()) {
      // Both status and search
      logger.debug('Service: Applying status + search filter:', status)
      whereCondition = and(
      ...
      // Only status
      logger.debug('Service: Applying status filter only:', status)
      whereCondition = eq(waitlist.status, status as string)
    } else if (search && search.trim()) {
      // Only search
      logger.debug('Service: Applying search filter only')
      whereCondition = like(waitlist.email, `%${search.trim()}%`)
    } else {
      logger.debug('Service: No filters applied, showing all entries')
    }

    // Log what filter is being applied
    logger.debug('Service: Where condition:', whereCondition ? 'applied' : 'none')
...
export async function approveWaitlistUser(
  ...
  try {
    ...
    try {
      const rendered = await renderEmail('waitlist-approval', {
        email: normalizedEmail,
        signupLink,
      })
      ...
      if (!emailResult.success) {
        logger.error('Error sending approval email:', emailResult.message)
...
export async function isUserApproved(email: string): Promise<boolean> {
  try {
    ...
    logger.error('Error checking if user is approved:', error)
    return false
  }
}
...
export async function markWaitlistUserAsSignedUp(
  ...
  try {
    ...
    logger.error('Error marking waitlist user as signed up:', error)
    return {
      success: false,
...
export async function approveBatchWaitlistUsers(emails: string[]): Promise<{
  ...
  try {
    ...
    const emailOptions = await Promise.all(
      users.map(async (user: typeof waitlist.$inferSelect) => {
        // Create a special signup token
        const token = await createToken({
          email: user.email,
          type: 'waitlist-approval',
    ...
    const results = users.map((user: typeof waitlist.$inferSelect, index: number) => {
    ...
    const successfulEmails = results
      .filter((result: { email: string; success: boolean }) => result.success)
      .map((result: { email: string; success: boolean }) => result.email)
    ...
    logger.error('Error approving batch waitlist users:', error)
    return {
      success: false,
