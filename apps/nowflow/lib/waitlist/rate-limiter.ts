...
// Delegates to the Cloudflare-aware helper so that `CF-Connecting-IP` wins
// over the spoofable `X-Forwarded-For` chain when the app is behind CF.
...
export async function isRateLimited(
  request: NextRequest,
  type: 'waitlist' | 'demo-request' = 'waitlist'
): Promise<{
  limited: boolean
  message?: string
