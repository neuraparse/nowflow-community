...
// NOTE: This is a per-instance cache; on multi-instance deploys cross-instance
// state lives in Redis. The map below is intentionally a best-effort local
// fallback only — see hasProcessedMessage / markMessageAsProcessed.
const inMemoryCache = new Map<
  string,
  { value: string; expiry: number | null; lastAccessed: number }
>()
...
function warnFallbackOnce() {
  ...
  fallbackWarned = true
  logger.warn(
    'Redis unavailable — using in-memory fallback cache. Cross-instance state will NOT be shared.'
  )
}
...
function evictIfOverCap() {
  ...
  // If still too large, remove least recently accessed entries (LRU)
  if (inMemoryCache.size > MAX_CACHE_SIZE) {
    const entries = Array.from(inMemoryCache.entries())
    entries.sort((a, b) => a[1].lastAccessed - b[1].lastAccessed)
    const keysToDelete = entries.slice(0, inMemoryCache.size - MAX_CACHE_SIZE).map((e) => e[0])
...
// Periodic TTL sweep so expired entries don't linger when no writes happen.
// .unref() so this never keeps the Node process alive.
if (typeof setInterval !== 'undefined') {
  const sweep = setInterval(() => {
    const now = Date.now()
    for (const [k, entry] of inMemoryCache.entries()) {
      if (entry.expiry && entry.expiry < now) inMemoryCache.delete(k)
    }
    evictIfOverCap()
...
// Test-only helpers (not exported in the public API surface but safe to use
// from co-located unit tests).
export const __inMemoryCacheForTests = inMemoryCache
export const __evictIfOverCapForTests = evictIfOverCap
...
export function getRedisClient(): Redis | null {
  ...
  try {
    ...
    globalRedisClient.on('connect', () => {
      logger.info('Redis main client connected (for publishing)', {
        connectionName: 'nowflow-main-client',
    ...
    globalRedisClient.on('close', () => {
      logger.warn('Redis main client connection lost', {
        connectionName: 'nowflow-main-client',
      })
    })
...
export async function markMessageAsProcessed(
  ...
  try {
    ...
    if (redis) {
      ...
      inMemoryCache.set(fullKey, { value: '1', expiry, lastAccessed: Date.now() })

      // Enforce LRU cap (also runs as a periodic sweep above).
      evictIfOverCap()
    }
  } catch (error) {
