...
export async function checkUsageStatus(userId: string): Promise<UsageData> {
  try {
    // In development and test, always return permissive limits to avoid external deps in tests
    if (!isProd || process.env.NODE_ENV === 'test') {
      // Get actual usage from the database for display purposes when possible
      try {
        ...
        // If db is mocked or unavailable in tests, still return permissive defaults
        return {
          percentUsed: 0,
          isWarning: false,
          isExceeded: false,
    ...
    // Production environment - use real limits
    const plan = await getEffectiveSubscriptionPlan(userId)
    const limit = plan.costLimit
...
export async function checkServerSideUsageLimits(userId: string): Promise<{
  ...
  try {
    ...
    return {
      ...
      currentUsage: 0,
      limit: 0,
      message: 'Unable to verify usage limits right now. Please try again shortly.',
    }
  }
}
