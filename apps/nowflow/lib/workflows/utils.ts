...
export async function updateWorkflowRunCounts(workflowId: string, runs: number = 1) {
  try {
    ...
    // Direct DB update - avoids self-referencing HTTP calls that can fail
    // due to URL misconfiguration or during deployment verification
    await db
    ...
    // Update user stats atomically with upsert
    if (workflow.userId) {
      await db
        .insert(userStats)
        .values({
        ...
        .onConflictDoUpdate({
          ...
          set: {
            totalManualExecutions: sql`${userStats.totalManualExecutions} + ${runs}`,
            lastActive: new Date(),
          },
        })
    }
