...
export function generateUUID(): string {
  // Try to use crypto.randomUUID if available (Node.js 16+ or modern browsers)
...
function getEncryptionKey(): Buffer {
  ...
  // Accepts either a 64-char hex string (used as-is) or any other non-empty
  // string (derived to 32 bytes via SHA-256 — cryptographically sound). The
  // only real problem is the hard-coded fallback, which we warn about once.
...
/**
 * Get a color for a workflow category
