...
try {
  ...
  // Create a fallback that just returns the input
  chalk = new Proxy(
    {},
    {
      get: () => (str: string) => str,
 ...
 * debugThrottleRate: Rate at which debug logs are shown (0.0 to 1.0)
 */
const LOG_CONFIG = {
  development: {
    enabled: true,
    minLevel: LogLevel.INFO, // Reduced from DEBUG to INFO to reduce log spam
    colorize: true,
    debugThrottleRate: 0.1, // Only show 10% of debug logs
  },
  production: {
    enabled: true,
    minLevel: LogLevel.WARN, // Only show WARN and ERROR in production
    colorize: false,
    debugThrottleRate: 0.0, // No debug logs in production
  },
  test: {
    enabled: false, // Disable logs in test environment
    minLevel: LogLevel.ERROR,
    colorize: false,
    debugThrottleRate: 0.0,
  },
}

// Get current environment
const ENV = process.env.NODE_ENV || 'development'
type LogConfigKey = keyof typeof LOG_CONFIG
let config =
  (LOG_CONFIG as Record<string, (typeof LOG_CONFIG)[LogConfigKey]>)[ENV] || LOG_CONFIG.development

// Allow environment variable overrides for log level
if (process.env.LOG_LEVEL) {
  const envLogLevel = process.env.LOG_LEVEL.toUpperCase()
  if (Object.values(LogLevel).includes(envLogLevel as LogLevel)) {
    config = {
      ...config,
      minLevel: envLogLevel as LogLevel,
    }
  }
}

// Allow environment variable override for debug throttle rate
if (process.env.DEBUG_LOG_RATE) {
  const rate = parseFloat(process.env.DEBUG_LOG_RATE)
  if (!isNaN(rate) && rate >= 0 && rate <= 1) {
    config = {
      ...config,
      debugThrottleRate: rate,
    }
  }
}
...
export class Logger {
   ...
   * These logs are throttled to prevent spam.
   *
   * Examples:
  ...
  debug(message: string, ...args: any[]) {
    // Apply throttling to debug logs to prevent spam
    if (config.debugThrottleRate && Math.random() > config.debugThrottleRate) {
      return
    }
