...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!session?.user?.id) {
      logger.warn(`[${requestId}] Unauthorized webhook test request`)
    ...
    // Find the webhook in the database and ensure it belongs to the current user.
    const webhooks = await db
      .select({ webhook })
      .from(webhook)
      .innerJoin(workflow, eq(webhook.workflowId, workflow.id))
      .where(and(eq(webhook.id, webhookId), eq(workflow.userId, session.user.id)))
      .limit(1)
    ...
    switch (provider) {
      case 'whatsapp': {
        ...
        return NextResponse.json({
          ...
          test: {
            ...
            responseMatchedChallenge: responseText === challenge,
          },
          message: success
      ...
      case 'generic': {
        ...
        if (requireAuth && token) {
          if (secretHeaderName) {
            curlCommand += ` -H "${secretHeaderName}: <redacted-token>"`
          } else {
            curlCommand += ` -H "Authorization: Bearer <redacted-token>"`
          }
        }
        ...
        return NextResponse.json({
          ...
          test: {
            ...
            headers: requireAuth
              ? secretHeaderName
                ? { [secretHeaderName]: '<redacted-token>' }
