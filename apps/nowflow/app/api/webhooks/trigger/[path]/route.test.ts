...
const consumeApiCallQuotaMock = vi.fn().mockResolvedValue({
  allowed: true,
  currentCalls: 1,
  limit: 2000,
})
const checkServerSideUsageLimitsMock = vi.fn().mockResolvedValue({
  isExceeded: false,
  currentUsage: 0,
  limit: 1000,
})
...
vi.mock('@/lib/api-rate-limits', () => ({
  ...
  consumeApiCallQuota: consumeApiCallQuotaMock,
}))
...
describe('Webhook Trigger API Route', () => {
  beforeEach(() => {
    ...
    consumeApiCallQuotaMock.mockResolvedValue({
      allowed: true,
      currentCalls: 1,
      limit: 2000,
    })
