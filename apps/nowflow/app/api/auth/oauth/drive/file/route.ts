...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      let userMessage = 'Failed to fetch file from Google Drive'
      if (response.status === 404) {
        userMessage = `File not found: ${fileId}.`
      } else if (response.status === 403) {
        userMessage = 'Access denied. You may not have permission to view this file.'
      } else if (response.status === 401) {
        userMessage = 'Authentication failed. Please reconnect your Google Drive account.'
