...
export async function POST(request: Request) {
  try {
    ...
    logger.debug(`Fetching Confluence pages from: ${url}`)
    ...
    logger.debug(`Found ${data.results?.length || 0} pages`)

    if (data.results && data.results.length > 0) {
      logger.debug('First few pages:')
      data.results.slice(0, 3).forEach((page: any) => {
        logger.debug(`- ${page.id}: ${page.title}`)
      })
    }
