...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Get the provider, providerId, and optional accountId from the request body
    ...
    // If a specific accountId is provided, delete only that account record
    if (accountId) {
      await db
        .delete(account)
        .where(and(eq(account.userId, session.user.id), eq(account.id, accountId)))
    } else if (providerId) {
      // If a specific providerId is provided, delete accounts matching that provider
      await db
        .delete(account)
