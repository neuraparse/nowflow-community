...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.error(`[${requestId}] Gmail API error response`, {
      ...
      try {
        ...
        return NextResponse.json(
          { error: error?.error?.message || error?.message || 'Gmail API error' },
          { status: response.status }
        )
      } catch (e) {
        return NextResponse.json({ error: 'Gmail API error' }, { status: response.status })
      }
    }
