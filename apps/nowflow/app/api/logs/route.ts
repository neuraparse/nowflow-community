  ...
  try {
    ...
    try {
      ...
      if (workflowIds.length > 1) {
        const workflowConditions = workflowIds.map((id: any) => eq(workflowLogs.workflowId, id))
      ...
      if (params.includeWorkflow === 'true' && logs.length > 0) {
        ...
        const uniqueWorkflowIds = [...new Set(logs.map((log: any) => log.workflowId))]
        ...
        const workflowMap = new Map(workflowData.map((w: any) => [w.id, w]))

        // Attach workflow data to each log
        const logsWithWorkflow = logs.map((log: any) => ({
          ...log,
