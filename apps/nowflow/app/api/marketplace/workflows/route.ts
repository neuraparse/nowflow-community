...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const sections = sectionParam
      ? sectionParam.split(',')
      : ['examples', 'popular', 'recent', 'byCategory']

    // Get examples if requested (only approved ones)
    if (sections.includes('examples')) {
      result.examples = await db
        .select(selectFields)
        .from(schema.marketplace)
        .where(
          and(eq(schema.marketplace.isExample, true), eq(schema.marketplace.status, 'approved'))
        )
        .orderBy(asc(schema.marketplace.exampleOrder))
        .limit(limit)
    }

    // Get popular items if requested (excluding examples, only approved ones)
    if (sections.includes('popular')) {
      result.popular = await db
        .select(selectFields)
        .from(schema.marketplace)
        .where(
          and(eq(schema.marketplace.isExample, false), eq(schema.marketplace.status, 'approved'))
        )
        .orderBy(desc(schema.marketplace.views))
        .limit(limit)
    }

    // Get recent items if requested (excluding examples, only approved ones)
      ...
      await Promise.all(
        categoriesToFetch.map(async (categoryValue) => {
          const categoryItems = await db
            ...
            .where(
              and(
                eq(schema.marketplace.category, categoryValue),
                eq(schema.marketplace.status, 'approved')
              )
            )
            .orderBy(desc(schema.marketplace.views))
            .limit(limit)
    ...
    if (includeState) {
      ...
      if (result.examples && result.examples.length > 0) {
        result.examples = transformSection(result.examples)
      }
