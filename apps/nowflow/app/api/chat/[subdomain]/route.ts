...
export async function POST(
  ...
  try {
    ...
    const { message, password, email, sessionToken, fingerprint } = parsedBody
    ...
    try {
      ...
      // Pass sessionToken and fingerprint for session persistence across requests
      ...
      // Provide user-friendly error messages for common workflow issues
      let userMessage = error.message || 'Failed to process request'

      if (error.message?.includes('Starter block must have at least one outgoing connection')) {
        userMessage =
          'This chat is not fully configured yet. The workflow needs to have blocks connected to process messages.'
      } else if (error.message?.includes('No AI block found')) {
        userMessage =
          'This chat is not configured to respond. Please add an AI block to the workflow.'
      } else if (error.message?.includes('API key')) {
        userMessage = 'This chat is temporarily unavailable. Please contact the administrator.'
      }

      return addCorsHeaders(createErrorResponse(userMessage, 500), request)
    }
  } catch (error: any) {
    logger.error(`[${requestId}] Error processing chat request:`, error)
    ...
    if (error.message?.includes('Starter block must have at least one outgoing connection')) {
      ...
      error.message?.includes('Provider fetch failed') ||
      error.message?.includes('ECONNREFUSED') ||
      error.message?.includes('fetch failed')
    ) {
      // Provider service unavailable - return 503 for retry
      userMessage = 'The AI service is temporarily unavailable. Please try again in a moment.'
      statusCode = 503
      logger.warn(`[${requestId}] Provider service unavailable - returning 503 for retry`)
    }

    return addCorsHeaders(createErrorResponse(userMessage, statusCode), request)
  }
}
