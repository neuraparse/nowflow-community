...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const chatUrl = isDevelopment
      ...
      : `https://${chatInstance[0].subdomain}.${APP_HOSTNAME}`
  ...
  try {
    ...
    try {
      ...
      const chatUrl = isDevelopment
        ...
        : `https://${updatedSubdomain}.${APP_HOSTNAME}`
...
export async function DELETE(
  ...
  try {
    ...
    logger.info(`DELETE /api/chat/edit/${chatId} - Request received`, {
      chatId,
      hasSession: !!session,
    ...
    logger.info(`DELETE /api/chat/edit/${chatId} - Chat lookup result`, {
      found: existingChat.length > 0,
      chatId,
    })

    if (existingChat.length === 0) {
      // Check if chat exists but belongs to different user
      const anyChat = await db.select().from(chat).where(eq(chat.id, chatId)).limit(1)
      logger.warn(`DELETE /api/chat/edit/${chatId} - Chat not found for user`, {
        chatExistsForOtherUser: anyChat.length > 0,
