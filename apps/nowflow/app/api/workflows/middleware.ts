...
import { canDeployWorkflow } from '@/lib/workflows/permissions'
import { getWorkflowById } from '@/lib/workflows/utils'
import { db } from '@/db'
import { apiKey } from '@/db/schema'
import { WorkflowCollaborator } from './[id]/collaborators/route'
...
export async function validateWorkflowAccess(
  ...
  try {
    ...
    if (requireDeployment) {
      ...
      const validApiKey = userApiKeys.some((k: any) => {
        try {
          const a = Buffer.from(k.key)
          const b = Buffer.from(apiKeyHeader!)
