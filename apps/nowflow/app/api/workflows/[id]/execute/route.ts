...
async function executeWorkflow(workflow: any, requestId: string, input?: any) {
  ...
  // Reserve API call quota only after usage access is confirmed.
  const apiCallCheck = await consumeApiCallQuota(workflow.userId)
  if (!apiCallCheck.allowed) {
    logger.warn(`[${requestId}] User ${workflow.userId} has exceeded API call limit`, {
      currentCalls: apiCallCheck.currentCalls,
      limit: apiCallCheck.limit,
    })
    throw new UsageLimitError(
      apiCallCheck.message || 'API call limit exceeded. Limit resets in 24 hours.'
    )
  }
  ...
  try {
    ...
    // Use block subBlocks as-is for API execution to avoid frontend store dependency
    const mergedStates = blocks
    ...
    if (!userEnv || !userEnv.variables) {
      logger.warn(
        `[${requestId}] No environment variables found for user: ${workflow.userId}, using defaults`
      )
    } else {
      // Parse and validate environment variables (ensures expected shape)
      EnvVarsSchema.parse(userEnv.variables)
    }
    ...
    const decryptedEnvVars = effectiveEnvVars
