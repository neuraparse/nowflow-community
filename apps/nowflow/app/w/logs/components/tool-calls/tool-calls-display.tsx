...
export function ToolCallsDisplay({ metadata }: ToolCallsDisplayProps) {
  if (!metadata.toolCalls || metadata.toolCalls.length === 0) {
    return <div className="text-sm text-zinc-400 dark:text-white/40">No tool calls recorded</div>
  }

  return (
    <div className="space-y-2">
      <div className="border rounded-md bg-zinc-100/40 dark:bg-white/[0.03]">
  ...
  return (
    <div
      className={cn(
        ...
        expanded ? 'bg-zinc-100/60 dark:bg-white/[0.04]' : ''
      )}
    >
      {/* Tool call header */}
      <div
        className="flex items-center p-2 cursor-pointer hover:bg-zinc-100/80 dark:hover:bg-white/[0.06] transition-colors"
      ...
      >
        ...
        <div className="flex-1 flex items-center">
          ...
          <div className="ml-auto flex items-center gap-3 text-xs text-zinc-400 dark:text-white/40">
      ...
      {/* Tool call details */}
      {expanded && (
        <div className="p-3 bg-zinc-100/40 dark:bg-white/[0.03] text-xs border-t">
          <div className="space-y-4">
            {/* Timing information */}
            <div className="grid grid-cols-2 gap-2">
              <div>
                <div className="text-zinc-400 dark:text-white/40 mb-1">Start Time</div>
              ...
              <div>
                <div className="text-zinc-400 dark:text-white/40 mb-1">End Time</div>
                <div className="font-mono relative group">
            ...
            {toolCall.input && (
              <div>
                <div className="text-zinc-400 dark:text-white/40 mb-1">Input</div>
                <pre className="bg-white dark:bg-slate-900 rounded p-2 overflow-auto max-h-32 relative group">
            ...
            {toolCall.status === 'success' && toolCall.output && (
              <div>
                <div className="text-zinc-400 dark:text-white/40 mb-1">Output</div>
                <pre className="bg-white dark:bg-slate-900 rounded p-2 overflow-auto max-h-32 relative group">
