...
const formatJsonContent = (content: string): React.ReactNode => {
  ...
  if (match) {
    ...
    return (
      <div className="w-full">
        <div className="text-sm font-medium mb-2 text-zinc-400 dark:text-white/40">
          {systemComment}
        </div>
        <div className="bg-zinc-100/60 dark:bg-white/[0.04] p-3 rounded-md relative group">
  ...
  return (
    <div className="bg-zinc-100/60 dark:bg-white/[0.04] p-3 rounded-md relative group w-full">
      <CopyButton text={formatted} className="h-7 w-7 z-10" />
      <pre className="text-sm whitespace-pre-wrap break-all w-full overflow-y-auto max-h-[500px] overflow-x-hidden">
        {formatted}
      </pre>
...
export function Sidebar({
  ...
  return (
    <div
      className={`fixed inset-y-0 right-0 bg-white dark:bg-slate-900 border-l transform ${
    ...
    >
      <div
        className="absolute left-[-4px] top-0 bottom-0 w-4 cursor-ew-resize hover:bg-zinc-100/50 dark:hover:bg-white/[0.04] z-50"
      ...
      {log && (
        <>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b flex-shrink-0">
            <h2 className="text-base font-medium font-logo text-zinc-800 dark:text-white">
              Log Details
            </h2>
          ...
          {/* Content */}
          <ScrollArea className="h-[calc(100dvh-64px-49px)] w-full" ref={scrollAreaRef}>
            <div className="p-4 space-y-4 w-full overflow-hidden pr-6">
              {/* Timestamp */}
              <div>
                <h3 className="text-xs font-medium font-logo text-zinc-400 dark:text-white/40 mb-1">
                  Timestamp
                </h3>
              ...
              {log.metadata?.toolCalls && log.metadata.toolCalls.length > 0 && (
                <div className="w-full">
                  ...
                  <div className="w-full overflow-x-hidden bg-zinc-100/60 dark:bg-white/[0.04] p-3 rounded-md">
              ...
              {hasCostInfo && log.metadata?.cost && (
                <div>
                  ...
                  <div className="rounded-md border overflow-hidden">
                    <div className="p-3 space-y-2">
                      {log.metadata.cost.model && (
                        <div className="flex justify-between items-center">
                          <span className="text-sm text-zinc-400 dark:text-white/40">Model:</span>
                          <span className="text-sm">{log.metadata.cost.model}</span>
                        </div>
                      )}
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-zinc-400 dark:text-white/40">Input:</span>
                        <span className="text-sm">{formatCost(log.metadata.cost.input || 0)}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-zinc-400 dark:text-white/40">Output:</span>
                        <span className="text-sm">{formatCost(log.metadata.cost.output || 0)}</span>
                      </div>
                      <div className="flex justify-between items-center border-t pt-2 mt-1">
                        <span className="text-sm text-zinc-400 dark:text-white/40">Total:</span>
                        <span className="text-sm text-zinc-800 dark:text-white">
                          {formatCost(log.metadata.cost.total || 0)}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-xs text-zinc-400 dark:text-white/40">Tokens:</span>
                    ...
                    {isWorkflowWithCost && (
                      <div className="border-t bg-zinc-100 dark:bg-white/[0.06] p-3 text-xs text-zinc-400 dark:text-white/40">
                        <p>
