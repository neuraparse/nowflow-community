...
export function TraceSpansDisplay({
  ...
  const safeTraceSpans = traceSpans ?? []
  ...
  const allSpanIds = useMemo(() => collectAllSpanIds(safeTraceSpans), [safeTraceSpans])

  if (safeTraceSpans.length === 0) {
    return <div className="text-sm text-zinc-400 dark:text-white/40">No trace data available</div>
  }
  ...
  const workflowStartTime = safeTraceSpans.reduce((earliest, span) => {
    const startTime = new Date(span.startTime).getTime()
    return startTime < earliest ? startTime : earliest
  }, Infinity)

  // Find the latest end time among all spans
  const workflowEndTime = safeTraceSpans.reduce((latest, span) => {
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between items-center mb-2">
        <div className="text-xs font-medium text-zinc-400 dark:text-white/40">Trace Spans</div>
        <button
          onClick={allExpanded ? handleCollapseAll : handleExpandAll}
          className="flex items-center gap-1 text-xs text-zinc-400 dark:text-white/40 hover:text-zinc-800 dark:hover:text-white transition-colors"
        ...
        {safeTraceSpans.map((span, index) => {
          ...
          return (
            <TraceSpanItem
              ...
              isLast={index === safeTraceSpans.length - 1}
...
function TraceSpanItem({
  ...
  const getSpanIcon = () => {
    ...
    return <Code className="h-3 w-3 text-zinc-400 dark:text-white/40" />
  }
        ...
        <div className="flex-1 flex items-center min-w-0 overflow-hidden gap-2">
          <div className="min-w-0 overflow-hidden flex-shrink">
            <div className="flex items-center space-x-2 mb-0.5">
              ...
              {depth > 0 && (
                <span className="text-xs text-zinc-400 dark:text-white/40 whitespace-nowrap flex-shrink-0">
              ...
              {depth === 0 && (
                <span
                  className="text-xs text-zinc-400 dark:text-white/40 whitespace-nowrap flex-shrink-0"
            ...
            <span className="text-xs text-zinc-400 dark:text-white/40 block">
          ...
          <div className="ml-auto flex items-center gap-2 flex-shrink-0">
            ...
            <div className="h-2 bg-zinc-100/80 dark:bg-white/[0.06] rounded-full overflow-hidden relative flex-shrink-0 hidden sm:block sm:w-24 md:w-32 lg:w-40 xl:w-56">
              <div
            ...
            <span className="text-xs text-zinc-400 dark:text-white/40 w-[65px] text-right font-mono tabular-nums flex-shrink-0">
              {`${duration}ms`}
            </span>
          </div>
        </div>
