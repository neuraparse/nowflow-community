...
export function EvalInput({ blockId, subBlockId }: EvalInputProps) {
  ...
  // 🔥 CRITICAL FIX: Normalize metrics to ensure all have valid range objects
  const normalizeMetric = (metric: any): EvalMetric => {
    return {
      ...
      range: {
        ...
        max: metric.range?.max ?? 1,
      },
    }
  }

  const metrics = (value || [DEFAULT_METRIC]).map(normalizeMetric)
