...
// In-memory credential cache to avoid redundant API calls when multiple
// CredentialSelectors mount simultaneously (e.g. right sidebar open).
// Entries expire after 10 seconds so fresh data is fetched on user actions.
const credentialCache = new Map<string, { data: Credential[]; ts: number }>()
...
export function CredentialSelector({
  ...
  // Fetch available credentials for this provider (with in-memory cache)
  const fetchCredentials = useCallback(
    async (bypassCache = false) => {
      // Check cache first (avoids N parallel API calls when sidebar opens)
      const cacheKey = effectiveProviderId
      if (!bypassCache) {
        const cached = credentialCache.get(cacheKey)
        if (cached && Date.now() - cached.ts < CACHE_TTL) {
          setCredentials(cached.data)
          setIsLoading(false)
          // Still run auto-select logic below with cached data
          const data = { credentials: cached.data }
          const currentSelectedId = selectedIdRef.current
          if (
            currentSelectedId &&
            !data.credentials.some((cred: Credential) => cred.id === currentSelectedId)
          ) {
            setSelectedId('')
          ...
          if (
            ...
            if (defaultCred) {
              ...
              selectedIdRef.current = defaultCred.id
              onChange(defaultCred.id)
            } else if (data.credentials.length === 1) {
              setSelectedId(data.credentials[0].id)
              selectedIdRef.current = data.credentials[0].id
      ...
      try {
        const response = await fetch(
          `/api/auth/oauth/credentials?provider=${effectiveProviderId}&t=${Date.now()}`,
          { cache: 'no-store' }
        )
        if (response.ok) {
          const data = await response.json()
          // Update cache
          credentialCache.set(cacheKey, { data: data.credentials, ts: Date.now() })
          setCredentials(data.credentials)

          // Use ref to get the latest selectedId (avoids stale closure)
          const currentSelectedId = selectedIdRef.current
  ...
  // Update local state and ref when external value changes
  useEffect(() => {
    setSelectedId(value)
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        fetchCredentials(true) // bypass cache - user returned from OAuth flow
      }
    }
  ...
  // Listen for credential changes dispatched from settings or other components
  useEffect(() => {
    const handleCredentialsChanged = () => {
      // Invalidate cache and refetch
      credentialCache.delete(effectiveProviderId)
  ...
  const handleOpenChange = (isOpen: boolean) => {
    setOpen(isOpen)
    if (isOpen) {
      // Fetch fresh credentials when opening the dropdown (bypass cache for user action)
      fetchCredentials(true)
    }
  }
  ...
  const handleSelect = (credentialId: string) => {
    logger.info('🔐 Credential selected:', {
      credentialId,
      provider: effectiveProviderId,
      serviceId: effectiveServiceId,
    })
    setSelectedId(credentialId)
    selectedIdRef.current = credentialId
