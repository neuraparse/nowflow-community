...
export function OAuthRequiredModal({
  ...
  const handleConnectDirectly = async () => {
    ...
    try {
      // Determine the appropriate serviceId and providerId
      providerId = getProviderIdFromServiceId(effectiveServiceId)
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md p-0 gap-0 rounded-[16px]">
        <DialogHeader className="px-6 pt-5 pb-4">
          ...
          <DialogDescription className="text-[12px] font-logo text-black/45 dark:text-white/50 mt-1 leading-relaxed">
        ...
        <div className="flex flex-col gap-4 px-6 py-4">
          <div className="flex items-center gap-4">
            <div className="rounded-lg bg-[#4A7A68]/[0.08] dark:bg-[#94B8A6]/[0.10] p-2">
              <ProviderIcon className="h-5 w-5" />
            </div>
            <div className="flex-1">
              <p className="text-[13px] font-logo font-medium text-black/80 dark:text-white/85">
          ...
          {displayScopes.length > 0 && (
            <div className="rounded-xl border border-black/[0.06] dark:border-white/[0.06] bg-black/[0.01] dark:bg-white/[0.02]">
              <div className="px-4 py-2.5 border-b border-black/[0.06] dark:border-white/[0.06]">
                <h4 className="text-[11px] font-logo font-semibold uppercase tracking-wider text-black/40 dark:text-white/45">
              ...
              <ul className="px-4 py-3 space-y-3">
                {displayScopes.map((scope) => (
                  <li key={scope} className="flex items-start gap-2 text-sm">
                    <div className="mt-0.5 rounded-md p-0.5 bg-[#4A7A68]/[0.08] dark:bg-[#94B8A6]/[0.10]">
                      <Check className="h-3 w-3 text-[#4A7A68] dark:text-[#94B8A6]" />
                    </div>
                    <span className="text-[12px] font-logo text-black/55 dark:text-white/60">
        ...
        <DialogFooter className="flex flex-col sm:flex-row gap-2 px-6 pb-5 pt-2">
          <Button
            variant="outline"
            onClick={onClose}
            className="sm:order-1 h-8 text-[12px] font-logo rounded-lg border-black/[0.06] dark:border-white/[0.06]"
          >
            Cancel
          </Button>
          <Button
            ...
            className="sm:order-3 h-8 text-[12px] font-logo font-semibold rounded-lg bg-[#4A7A68] hover:bg-[#3d6a59] dark:bg-[#94B8A6]/90 dark:hover:bg-[#94B8A6] text-white dark:text-[#1b1b1b]"
          >
            Connect Now
          </Button>
          <Button
            ...
            className="sm:order-2 h-8 text-[12px] font-logo rounded-lg border-black/[0.08] dark:border-white/[0.08] text-black/60 dark:text-white/65"
          >
            Go to Settings
          </Button>
        </DialogFooter>
