...
export function WebhookModal({
  ...
  const formattedPath = webhookPath && webhookPath.trim() !== '' ? webhookPath : generateUUID()
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={handleClose}>
        <DialogContent
          className="sm:max-w-[800px] flex flex-col p-0 gap-0 max-h-[90vh] overflow-hidden rounded-[16px]"
          hideCloseButton
        >
          ...
          <Tabs defaultValue="config" className="flex-1 flex flex-col overflow-hidden">
            <TabsList className="mx-6 mt-4 grid w-auto grid-cols-3 rounded-lg font-logo">
            ...
            {webhookId && (
              <>
                <TabsContent value="stats" className="flex-1 overflow-y-auto px-6 pb-6 mt-4">
          ...
          <DialogFooter className="px-6 pb-5 w-full border-t border-black/[0.06] dark:border-white/[0.06] pt-4">
            <WebhookDialogFooter
              webhookId={webhookId}
