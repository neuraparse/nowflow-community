...
export function FileSelectorInput({ blockId, subBlock, disabled = false }: FileSelectorInputProps) {
  ...
  // For Confluence and Jira, we need the domain and credentials (read at call time)
  const store = useSubBlockStore.getState()
  const domain = isConfluence || isJira ? (store.getValue(blockId, 'domain') as string) || '' : ''
  const credentials =
    isConfluence || isJira ? (store.getValue(blockId, 'credential') as string) || '' : ''

  // Get the current value from the store (read once on mount)
  useEffect(() => {
    const s = useSubBlockStore.getState()
    const value = s.getValue(blockId, subBlock.id)
  ...
  }, [blockId, subBlock.id, isJira])

  // Handle file selection
  const handleFileChange = (fileId: string, info?: any) => {
    setSelectedFileId(fileId)
    setFileInfo(info || null)
    useSubBlockStore.getState().setValue(blockId, subBlock.id, fileId)
  }
  ...
  const handleIssueChange = (issueKey: string, info?: JiraIssueInfo) => {
    ...
    useSubBlockStore.getState().setValue(blockId, subBlock.id, issueKey)
