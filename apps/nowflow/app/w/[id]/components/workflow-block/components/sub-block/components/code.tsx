...
interface CodeProps {
  ...
  placeholder?: string
  language?: 'javascript' | 'json' | 'text' | 'graphql'
...
export function Code({
  ...
  return (
    <>
      ...
      <div
        className={cn(
          'relative min-h-[100px] rounded-lg border border-border/60 bg-muted/20 font-mono text-sm group',
      ...
      >
        ...
        <div
          className="absolute left-0 top-0 bottom-0 w-[32px] bg-muted/40 border-r border-border/30 flex flex-col items-end pr-2 pt-3 select-none overflow-hidden rounded-l-lg"
          aria-hidden="true"
        >
          {renderLineNumbers()}
        </div>
        ...
        >
          {code.length === 0 && !isCollapsed && (
            <div
              className="absolute left-[48px] top-[12px] text-muted-foreground/40 select-none pointer-events-none text-xs italic truncate pr-12"
              style={{ maxWidth: 'calc(100% - 60px)' }}
              title={placeholder}
            >
              {placeholder.length > 50 ? `${placeholder.slice(0, 50)}...` : placeholder}
            </div>
          )}
          ...
          <Editor
            ...
            textareaClassName={cn(
              'unstyled-editor-field code-editor-textarea',
