...
export function ConditionInput({ blockId, subBlockId, isConnecting }: ConditionInputProps) {
  ...
  // Guard: track whether the current blocks came from store data (real) vs defaults (generated).
  // Prevents writing generated defaults to the store on sidebar open — that write triggers
  // live-validation which clears validation errors before the user has fixed anything.
  const isDefaultOnlyRef = useRef(false)
  ...
  const safeParseJSON = (jsonString: string | null): ConditionalBlock[] | null => {
    ...
    // Only attempt parsing if the string looks like a JSON array
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (storeValueStr === null) {
        ...
        // If we haven't initialized yet, set default blocks for display only
        // isDefaultOnlyRef = true prevents these from being written to the store
        setConditionalBlocks(createDefaultBlocks())
        hasInitializedRef.current = true
        isDefaultOnlyRef.current = true
        setIsReady(true)
        isSyncingFromStoreRef.current = false
        return
      }
      ...
      if (parsedBlocks) {
        ...
        hasInitializedRef.current = true
        isDefaultOnlyRef.current = false // Real data loaded — safe to write
        if (!isReady) setIsReady(true)
      } else if (!hasInitializedRef.current) {
        // Only set default blocks if we haven't initialized yet — display only, not written to store
        setConditionalBlocks(createDefaultBlocks())
        hasInitializedRef.current = true
        isDefaultOnlyRef.current = true // Default only — do NOT write to store
        setIsReady(true)
      }
    } finally {
  ...
  useEffect(() => {
    ...
    // Skip writing when blocks were initialized from generated defaults (not real store data).
    // Writing generated defaults triggers live-validation which clears validation errors
    // before the user has actually configured the block.
    if (isDefaultOnlyRef.current) {
      // Check if the current value has deviated from the generated defaults
      // (meaning user has actually interacted — added a condition, typed a value, etc.)
      const defaultValue = JSON.stringify(createDefaultBlocks())
      if (newValue === defaultValue) return // Still pure defaults — skip
      // User changed something — allow writes from now on
      isDefaultOnlyRef.current = false
    }
