...
import { AlertCircle, Check, ChevronDown, ExternalLink, FileIcon, RefreshCw, X } from 'lucide-react'
...
export function GoogleDrivePicker({
  ...
  const fetchFileById = useCallback(
    async (fileId: string) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          let errorMessage = 'File not found or access denied'
          ...
          // Set error state and clear the invalid file selection
        ...
        setFileError('Failed to load file information')
        setSelectedFile(null)
        setSelectedFileId('')
        onChange('', undefined)
        onFileInfoChange?.(null)
    ...
    [selectedCredentialId, onFileInfoChange, onChange]
  )
  ...
  useEffect(() => {
    ...
    if (value && selectedCredentialId && !selectedFile && !fileError) {
      fetchFileById(value)
    }
  }, [value, selectedCredentialId, selectedFile, fileError, fetchFileById])

  // Keep internal selectedFileId in sync with the value prop
  useEffect(() => {
    if (value !== selectedFileId) {
      setSelectedFileId(value)
      // Clear the selected file when value changes to trigger a fresh fetch
      setSelectedFile(null)
      setFileError(null)
    }
          ...
          <div className="mt-2 rounded-md border border-destructive/50 bg-destructive/10 p-3">
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-destructive">File Error</p>
