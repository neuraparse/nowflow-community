...
function calculateLoopBounds(loop: Loop, blocks: Record<string, any>) {
  ...
  const bound = loopBlocks.reduce(
    (acc, block) => {
      // React Flow runs with `nodeOrigin={[0.5, 0.5]}` on the workflow canvas,
      // so block positions are visual centers rather than top-left corners.
      const blockWidth =
        typeof block.width === 'number' && block.width > 0 ? block.width : block.isWide ? 480 : 320
      const blockHeight = typeof block.height === 'number' && block.height > 0 ? block.height : 200
      const left = block.position.x - blockWidth / 2
      const top = block.position.y - blockHeight / 2
      const right = block.position.x + blockWidth / 2
      const bottom = block.position.y + blockHeight / 2
...
export function createLoopNode({ loopId, loop, blocks }: WorkflowLoopProps) {
  ...
  // React Flow is configured with `nodeOrigin={[0.5, 0.5]}` in the workflow canvas,
  // so group/container nodes must be positioned by their visual center.
  const loopCenter = {
    x: loopBounds.x + loopBounds.width / 2,
    y: loopBounds.y + loopBounds.height / 2,
  }
  ...
  const loopNode = {
    ...
    type: 'group',
    position: loopCenter,
    className: 'workflow-loop-container',
    style: {
      border: '1px solid rgba(148, 163, 184, 0.2)',
      borderRadius: '12px',
      background: 'linear-gradient(180deg, rgba(10, 13, 15, 0.14), rgba(5, 7, 9, 0.08))',
