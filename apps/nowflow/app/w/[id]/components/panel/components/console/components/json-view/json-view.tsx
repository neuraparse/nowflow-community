...
const logger = createLogger('json-view')
  ...
  return (
    <span>
      ...
      <Button
        variant="link"
        size="sm"
        className="px-1 h-auto text-xs text-zinc-400 dark:text-white/40 hover:text-zinc-800 dark:hover:text-white"
        onClick={(e) => {
          e.stopPropagation()
...
const copyToClipboard = async (data: any) => {
  try {
    ...
    const success = await safeCopy(stringified)
    if (!success) {
...
const ImagePreview = ({
  ...
  if (!hasValidData) {
    return <div className="my-2 text-zinc-400 dark:text-white/40">Image data unavailable</div>
  }

  if (loadError) {
    return <div className="my-2 text-zinc-400 dark:text-white/40">Failed to load image</div>
  }
...
export const JSONView = ({ data, level = 0, initiallyExpanded = false }: JSONViewProps) => {
  ...
  if (data === null) return <span className="text-zinc-400 dark:text-white/40">null</span>
  ...
  if (isResponseWithImage) {
    ...
    return (
      <div className="relative" onContextMenu={handleContextMenu}>
        <span
          className="cursor-pointer select-none inline-flex items-center text-zinc-400 dark:text-white/40"
          onClick={(e) => {
            e.stopPropagation()
            ...
            {Object.entries(data).map(([key, value], index) => {
              ...
              return (
                <div key={key} className="break-all">
                  <span className="text-zinc-400 dark:text-white/40">{key}</span>:{' '}
                  {isImageKey ? (
                    <div>
  ...
  if (isEmpty) {
    return <span className="text-zinc-400 dark:text-white/40">{isArray ? '[]' : '{}'}</span>
  }
  ...
  return (
    <div className="relative" onContextMenu={handleContextMenu}>
      ...
      <span className="text-zinc-400 dark:text-white/40">{isArray ? ']' : '}'}</span>
    </div>
  )
}
