...
export class ToolTester<P = any, R = any> {
  ...
  async execute(params: P, skipProxy = true): Promise<ToolResponse> {
    ...
    try {
      // Resolve method: prefer params.method, then tool.request.method (function or string)
      const methodFromTool =
        typeof this.tool.request.method === 'function'
          ? this.tool.request.method(params as any)
          : this.tool.request.method
      const method = (params as any)?.method || methodFromTool
