...
export const writeTool: ToolConfig<GoogleDocsToolParams, GoogleDocsWriteResponse> = {
  ...
  transformResponse: async (response: Response) => {
    if (!response.ok) {
      let errorMessage = `Failed to write to Google Docs document (Status: ${response.status})`
      try {
        ...
        if (errorData.error) {
          ...
          if (err.status === 'INVALID_ARGUMENT') {
            errorMessage +=
              '. This may be caused by: 1) Invalid document ID, 2) The file is not a native Google Docs document, or 3) Missing OAuth scope - try reconnecting your Google account.'
          } else if (err.status === 'PERMISSION_DENIED' || err.code === 403) {
            errorMessage += '. Please check if you have write access to this document.'
          }
        }
      } catch {
        try {
