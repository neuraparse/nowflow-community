...
export const createTool: ToolConfig<GoogleDocsToolParams, GoogleDocsCreateResponse> = {
  ...
  transformResponse: async (response: Response) => {
    if (!response.ok) {
      let errorMessage = `Failed to create Google Docs document (Status: ${response.status})`
      try {
        ...
        if (errorData.error) {
          ...
          if (err.status === 'INVALID_ARGUMENT') {
            errorMessage +=
              '. This may be caused by missing OAuth scope - try reconnecting your Google account.'
          } else if (err.status === 'PERMISSION_DENIED' || err.code === 403) {
            errorMessage += '. Please check your Google account permissions.'
          }
        }
      } catch {
        try {
