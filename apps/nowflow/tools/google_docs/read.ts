...
export const readTool: ToolConfig<GoogleDocsToolParams, GoogleDocsReadResponse> = {
  ...
  transformResponse: async (response: Response) => {
    if (!response.ok) {
      let errorMessage = `Failed to read Google Docs document (Status: ${response.status})`
      try {
        const errorData = await response.json()
        if (errorData.error) {
          const err = errorData.error
          errorMessage = err.message || err.status || JSON.stringify(err)
          // Add helpful hints for common errors
          if (err.status === 'INVALID_ARGUMENT') {
            errorMessage +=
              '. This may be caused by: 1) Invalid document ID, 2) The file is not a native Google Docs document (PDF, Word, etc.), or 3) Missing OAuth scope - try reconnecting your Google account.'
          } else if (err.status === 'PERMISSION_DENIED' || err.code === 403) {
            errorMessage +=
              '. Please check if you have access to this document and try reconnecting your Google account with the required permissions.'
          } else if (err.status === 'NOT_FOUND' || err.code === 404) {
            errorMessage +=
              '. The document was not found. Please verify the document ID is correct.'
          }
        }
      } catch {
