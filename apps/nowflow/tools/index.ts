...
export async function executeTool(
  ...
  try {
    ...
    // Skip validation if skipProxy=true (validation already done in proxy route)
    if (!skipProxy) {
    ...
    logger.info(`[executeTool] Parameters for ${toolId}:`, {
      hasCredential: !!contextParams.credential,
      hasWorkflowId: !!contextParams._context?.workflowId,
      workflowId: contextParams._context?.workflowId,
      paramKeys: Object.keys(contextParams),
    })

    // Determine if this tool will use the proxy or not
    // Only exchange credential for access token if NOT using proxy
    // For proxy requests, the server will handle credential exchange
    const willUseProxy = !tool.request.isInternalRoute && !skipProxy && !tool.directExecution

    // If we have a credential parameter and NOT using proxy, fetch the access token
    if (contextParams.credential && !willUseProxy) {
      logger.info(
        `[executeTool] Credential found for ${toolId}, fetching access token (client-side).`
      )
      try {
        // Use window.location.origin in browser, fallback to env variable
        const baseUrl =
        ...
        if (!response.ok) {
          ...
          logger.error('[executeTool] Token fetch failed:', {
    ...
    } else if (contextParams.credential && willUseProxy) {
      logger.info(
        `[executeTool] Credential found for ${toolId}, will be exchanged on server-side via proxy.`
      )
    }
    ...
    if (!response.ok) {
      ...
      try {
        ...
        errorDetails = {
          ...errorDetails,
          code: errorJson.code,
          error: typeof errorJson.error === 'string' ? errorJson.error : undefined,
        }
      } catch {
        // Keep raw provider responses out of logs/UI; they may contain request context.
      }
